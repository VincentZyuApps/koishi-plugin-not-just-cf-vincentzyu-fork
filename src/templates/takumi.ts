...
function getRenderer(fontPath?: string): Renderer {
  if (!fontPath || !existsSync(fontPath)) {
    throw new Error('Takumi 渲染需要有效字体文件，请配置 imageFontPath 或使用自动下载的 LXGW 字体。')
  }
  const key = fontPath
...
export async function renderContestImage(contests: Contest[], options: RenderOptions): Promise<Buffer> {
  ...
  const root = container({
    ...
    children: [
      buildHeader(title, subtitle, contests, t),
